...
impl<T: PartialOrd + Clone> LinkedList<T> {
    ...
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self {
        ...
        macro_rules! get_node_next {
            ($node:expr) => {
                $node.map(|ptr| unsafe { (*ptr.as_ptr()).next }).unwrap()
            };
        }
        while current_a.is_some() || current_b.is_some() {
            match (get_node_val(&current_a), get_node_val(&current_b)) {
                (Some(val_a), Some(val_b)) => {
                    if val_a < val_b {
                        list_c.add(val_a.clone());
                        current_a = get_node_next!(current_a);
                    } else {
                        list_c.add(val_b.clone());
                        current_b = get_node_next!(current_b);
                    }
                }
                (Some(val_a), None) => {
                    list_c.add(val_a.clone());
